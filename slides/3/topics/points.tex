...
\item<6-> 全局的对象借出可以不用归还，其他无论是栈还是堆上分配内存的对象借出时都需要考虑归还，
不能把自己或者自己的一部分借给借来的对象，或者是借给生命周期无法确定是否小于自己的对象。
\end{enumerate}
}
...
\item<2-> MVVM模式中的属性、通知、命令绑定都要看成是这种借-还的过程。
\item<3-> Window和View层中GUI库的对象使用指针绑定数据，也是一种借用，要注意各成员对象的析构顺序，
若借用者在借出者之后释放，也需要先归还。
\end{enumerate}
}
...
\item<4-> 集合循环语句，std::ranges和std::string_view，流式管道，并发，协程
