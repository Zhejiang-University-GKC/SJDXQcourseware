...
\item<2-> 需要分析生命周期，对象间生命周期相同的借出一般可以不归还。
\item<3-> 把自己或者自己的一部分借给借来的对象时，要分析生命周期，借来的对象先归还自身借来的对象，
再归还自己。
\item<4-> 不能把自己或者自己的一部分借给生命周期无法确定是否小于自己的对象。
\item<5-> 若在析构中调用借来的对象，要分析析构的顺序，决定是否先归还。
\end{enumerate}
}

\frame{\frametitle{指针}
\begin{enumerate}
\item<1-> 函数、方法调用参数列表中的引用或指针型参数也要看成是一种借用过程，
函数或方法结束时其生命周期也结束，即自动归还。
要注意分析函数或方法在多个线程中被调用时这些借用的指针是否有效。
\item<2-> 借出关系很复杂时，在借出者被修改后，直接或间接(如通过属性改变通知)更新被借者的借来关系。
\item<3-> 某个对象实例或对象成员变量内部有借出时，如果它自己被聚合到其他对象实例中，
特别是直接或间接地成为容器对象（动态数组，链表，哈希表，红黑树）的元素时，
要分析指针是否有变化，防止出现野指针。
\end{enumerate}
}
