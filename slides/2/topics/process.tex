...
\frame{\frametitle{什么是协程？}
\begin{enumerate}
\item<1-> 协程（绿色线程，微线程），纤程，管程。
\item<2-> 在线程或进程中创建执行。
\item<3-> 不需要操作系统来进行资源调度和切换。
\item<4-> 高效利用线程时间，减少阻塞等待。
\end{enumerate}
}
...
\frame{\frametitle{其他}
在多线程的环境下抛出异常，可能在任意线程的上下文中执行异常处理代码，
一般不用做特别的考虑。只有在异常处理代码与线程上下文高度相关的特殊情形下，
才需要特别处理。
}
