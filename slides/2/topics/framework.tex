...
或者不做转换直接触发属性改变的通知。此外还可以调用其他Model对象的方法。
\end{itemize}
}
...
\item<5-> Window、View可以绑定来自一个或多个ViewModel对象的属性、命令。
\item<6-> 如有共用的各层对象，也可以由应用类直接管理。
\end{itemize}
}
...
\item<2-> View层可显示数据的改变，可以是View层持有的数据改变，也可以是ViewModel层持有的数据改变，或者Model层注入给ViewModel层的数据改变。
\item<3-> View层可显示数据的改变，根据业务的需要，通过命令来调用ViewModel层的方法，并根据各层数据不同的持有情况来决定是否要携带改变后的数据参数。
此时Model层或者ViewModel层不能再发出属性改变通知给发出命令的View对象。
\item<4-> ViewModel是一个或多个Model的中介者，提供针对Model的事件接收器，调用一个或多个Model的方法。
\item<5-> ViewModel是Model和View之间解开耦合的中间层。
\item<6-> 根据数据在内存中的各层对象占用情况，以及运行时的使用情况，动态地释放ViewModel和Model里的属性数据。
\end{enumerate}
}
...
Web前端目前流行js开发，也正从MVC模式转向MVVM模式。
}
